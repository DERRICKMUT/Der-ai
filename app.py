...
def get_high_impact_news():
    try:
        ...
        return [{
            'time': event.get('time', ''), 'currency': event.get('country', ''),
            'event': event.get('event', ''), 'impact': 'HIGH',
            'forecast': event.get('forecast', ''), 'previous': event.get('previous', '')
        } for event in data if event.get('date') == today and event.get('impact') == '3']
    except Exception as e:
...
def find_swings(df, window=5):
    ...
    return {"recent_swing_highs": [round(p, 5) for p in swing_highs], "recent_swing_lows": [round(p, 5) for p in swing_lows]}
...
def call_gpt(system_prompt: str, user_content: list, max_tokens: int = 2000) -> dict:
    ...
    payload = {
        ...
        "messages": [
            {"role": "system", "content": system_prompt + "\n\nCRITICAL: Output ONLY valid JSON. NO markdown code blocks."},
...
def analyze_symbol_premium(symbol):
    try:
        ...
        for tf, df in mtf_data.items():
            ...
            swings = find_swings(df) # ADDED: Get exact swing levels for TP calculation
...
💰 <b>ENTRY:</b> {analysis.get('entry', 'N/A')} | 🛑 <b>SL:</b> {analysis.get('stop_loss', 'N/A')}
...
🔍 <b>CONFLUENCE:</b> {', '.join(analysis.get('timeframes_aligned', []))} | OBs: {len(analysis.get('order_blocks', []))} | FVGs: {len(analysis.get('fvg_zones', []))}
...
sensitivity = st.sidebar.select_slider("Minimum Confluence Score", options=[80, 85, 90], value=85)
...
with tab1:
    ...
    def process_symbol_result(result, symbol, is_auto=False):
        if result and 'error' in result and 'RATE_LIMIT' in str(result.get('error')):
            msg = f"⏳ **{symbol}**: Groq free tier daily token limit reached. The app will automatically resume when the limit resets."
...
with tab3:
    ...
    st.markdown("This tab logs all app activities, including exactly *why* signals were rejected.")
